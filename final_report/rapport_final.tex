...
Sur le rerun du notebook~: $\widehat T_{\text{1 proc.}}=12{,}48$,
IC $=[12{,}43;\,12{,}53]$~; $\widehat T_{\min}=8{,}47$,
IC $=[8{,}44;\,8{,}50]$.
...
\midrule
$\E[\tau]$   & $12{,}500$ & $12{,}517$ & $+0{,}14\,\%$ \\
$\Var(\tau)$ & $62{,}500$ & $62{,}706$ & $+0{,}33\,\%$ \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
Kolmogorov-Smirnov (IG vs simulation) & $0{,}0055$ & $0{,}0971$ & Non-rejet à $5\,\%$ \\
\bottomrule
\end{tabular}
\end{center}

La statistique KS de $0{,}0055$ est nettement inférieure au seuil critique,
et la $p$-valeur de $0{,}0971$ confirme qu'il n'y a aucune raison statistique
...
La moyenne est restituée à $+0{,}14\,\%$, la variance à $+0{,}33\,\%$,
et le test KS ne rejette pas l'adéquation ($p=0{,}0971$). L'approximation
...
Les surfaces présentent le comportement attendu~: $\E[\min(\tau_a,\tau_b)]$ croît régulièrement avec les deux volumes initiaux, et la probabilité de ruine asymétrique respecte la symétrie diagonale ($p\approx1/2$ lorsque $q_0^a=q_0^b$). Pour $M=300$ trajectoires par point de grille, les intervalles de confiance restent de l'ordre de $\pm2\,\%$, ce qui valide ces surfaces comme outil opérationnel pour évaluer rapidement le risque d'épuisement en fonction des conditions initiales.
...
Le processus de Hawkes produit une dynamique qualitativement différente du cas
Poisson~: l'auto-excitation génère des avalanches d'annulations qui se manifestent
sous forme de descentes rapides et abruptes de $q(t)$, interrompues par des périodes
de stabilité relative lorsque l'intensité se détend vers $\mu^-$.
...
La figure~\ref{fig:hawkes_dyn} illustre le mécanisme d'avalanche~: après une annulation,
$\lambda^-(t)$ bondit d'une quantité $\alpha$, puis relaxe exponentiellement vers $\mu^-$
au taux $\beta$. Si une nouvelle annulation survient pendant cette période de relaxation,
le pic s'additionne, créant la possibilité de cascades. Cet effet modifie profondément
la distribution du temps d'arrêt, comme le montre la figure suivante.
...
La comparaison avec le Poisson de fond (paramètre $\lambda^-=\mu^-$) met en évidence
l'effet double de l'auto-excitation~: les avalanches créent une masse supplémentaire
pour les petites valeurs de $\tau$ (épuisements très rapides), tandis que les périodes
de faible intensité entre cascades s'étendent la queue droite. La distribution du Hawkes
est donc plus étalée et moins régulière que celle du Poisson.
...
Pour la simulation du système joint, l'algorithme de thinning d'Ogata est étendu
en maintenant une borne globale sur l'intensité totale~:
$\bar\Lambda = \bar\la+\bar\lb+\lambda^+_a+\lambda^+_b$.
À chaque pas de temps, un événement candidat est tiré selon cette borne globale,
puis attribué à l'un des quatre flux (annulation ask, annulation bid, insertion
ask, insertion bid) proportionnellement aux intensités effectives du moment,
avec rejet si le candidat tombe dans le complément. Cette approche est exacte
et préserve intégralement la mémoire Hawkes des deux files.
...
Au coin symétrique $(q_0^a,q_0^b)=(10,10)$, le temps moyen d'épuisement vaut
$\E[\min(\tau_a,\tau_b)]=5{,}83$, à comparer à $8{,}32$ pour le modèle de Poisson
équivalent, soit une réduction de $30\,\%$ (facteur $\approx0{,}70$). Ce raccourcissement
s'explique par le couplage croisé excitateur~: chaque événement sur l'une des files
accélère simultanément l'autre, de sorte que les deux files tendent à s'épuiser
plus rapidement ensemble que si elles évoluaient indépendamment.
...
\paragraph{Interprétation.}
Trois effets successifs ressortent de cette comparaison. Premièrement, le simple
passage du processus de Poisson individuel au minimum de deux processus indépendants
divise la moyenne par $\approx1{,}45$~: cet effet est purement combinatoire et ne fait
intervenir aucune mémoire. Deuxièmement, le Hawkes individuel réduit davantage la médiane
que la moyenne~: les avalanches d'annulations génèrent des réalisations très courtes
(queue gauche épaisse) qui abaissent la médiane, tandis que la queue droite reste épaisse
car les rares trajectoires sans excitation sont longues. Troisièmement, le couplage
croisé excitateur combine et amplifie ces deux effets~: la réactivité mutuelle des deux
files réduit à la fois la médiane et la moyenne, portant le facteur de réduction global
à $\approx2{,}1$ par rapport au Poisson individuel.

\subsection{Processus auxiliaire \texorpdfstring{$q_2$}{q2} au niveau adjacent}

Lorsque la première limite ask s'épuise, le carnet d'ordres recharge automatiquement
la deuxième limite~: il est donc pertinent d'étudier la distribution du volume
$q_{a,2}$ au niveau adjacent à l'instant d'épuisement $\tau=\min(\tau_a,\tau_b)$.
On simule ce processus auxiliaire en parallèle, en conditionnant sur le côté qui
s'épuise en premier. Cette distribution conditionnelle permet de calibrer l'état
initial du prochain cycle lors du passage au modèle de prix à quatre limites.
...
Un modèle plus réaliste consiste à coupler l'intensité d'insertion du niveau adjacent
au flux des décréments de la première limite~: chaque annulation ask signale une pression
vendeuse croissante, ce qui stimule les market makers à replenish le niveau suivant.
On modélise cela par
$\lambda^+_2(t) = \mu^+_2 + \sum_{T_i^{\downarrow,a}<t} a\,e^{-b(t-T_i^{\downarrow,a})}$,
où les coefficients $a$ et $b$ contrôlent respectivement l'amplitude et le taux de
relaxation de cette excitation croisée vers le niveau adjacent.
...
\midrule
Symétrique  & $0{,}518$ & $0{,}482$ & $+0{,}0185$ \\
Asymétrique & $0{,}738$ & $0{,}262$ & $+0{,}2375$ \\
\bottomrule
\end{tabular}
\end{center}
...
En cas symétrique, $\hat p_\uparrow = 0{,}518 \approx 1/2$ et la dérive est
négligeable ($+0{,}0185$ tick/cycle), confirmant le comportement de martingale.
En cas asymétrique, la dérive empirique de $+0{,}2375$ tick/cycle coïncide avec
la prédiction $\hat p_\uparrow-1/2=0{,}238$. Le modèle est donc \emph{identifiable}
...
\begin{enumerate}
  ...
  \item La largeur des IC à 95\,\% décroît également en $1/\sqrt N$
    conformément à la théorie asymptotique.
\end{enumerate}
...
\begin{figure}[H]
\centering
\reportfig[width=0.75\linewidth]{ams_convergence.png}
\caption{Graphe semi-log (gauche) et log-log (droite) de l'écart-type de $\hat P_\mathrm{AMS}$ en fonction de $N$ sur l'événement bid rare $\tau_b<\tau_a$ ($\mu_a^-=3{,}0$, $\mu_b^-=1{,}2$). La droite log-log a une pente $\approx-0{,}5$, confirmant la décroissance en $1/\sqrt N$ prédite par la Proposition~\ref{prop:ams_clt}. \textit{Horiz.~: $N$~; vert.~: écart-type.}}
\label{fig:ams_conv}
\end{figure}

\paragraph{Résultats quantitatifs.}

La pente log-log mesurée sur la figure est $\approx-0{,}5$, ce qui
...
de $0{,}349$ pour $N=50$ à $0{,}048$ pour $N=3200$, soit une réduction d'un
facteur~7 sur la plage considérée. Cette convergence rapide découle directement
de la structure de l'algorithme~: en décomposant l'événement rare en paliers
successifs et en réorientant les particules vers des états progressivement
plus proches de l'épuisement bid, l'AMS guide efficacement la simulation
là où le Monte-Carlo naïf gaspillerait la quasi-totalité des trajectoires
sur des réalisations ne franchissant pas le premier palier.
...
$c_1\simeq0{,}142$, $c_2\simeq0{,}043$, $c_3\simeq0{,}031$, etc., donc
...
Le rerun donne $p_1\simeq0{,}142$ dans le régime asymétrique. Les probabilités
AMS sont~:
...
\caption{Gauche~: estimation de $\hat P(\tau_b<\tau_a)$ en fonction de $\theta$,
comparée aux références splitting AMS (tiret orange, $0{,}1387$) et MC direct
(pointillé bleu, $0{,}1390$). Droite~: coefficient de variation empirique de l'IS,
avec $\theta^*\approx0{,}30$ marqué en vert. \textit{Horiz.~: $\theta$~; vert.~:
probabilité / CV.}}
\label{fig:is}
\end{figure}
...
\begin{enumerate}
  \item \textbf{Minimum net à $\theta^*\approx0{,}30$}~: pour les petits tilts
    ($0\le\theta\le0{,}30$), l'estimateur reste stable et converge vers les
    références AMS et MC direct. Le CV passe de $2{,}52$ à $0$ à $2{,}05$,
    une réduction modeste mais significative.
  \item \textbf{Effondrement rapide au-delà de $\theta^*$}~: dès $\theta=0{,}60$,
    le CV bondit à $10$ et l'estimateur commence à déraper~; à $\theta=1{,}80$,
    le CV atteint $109$ et $\hat p$ s'effondre, signalant une dégénérescence
    des poids de vraisemblance.
\end{enumerate}

\paragraph{Résultats quantitatifs.}
La grille du rerun ($N=30\,000$ trajectoires par point) donne un minimum empirique
clairement identifiable à $\theta^*\approx0{,}30$~:
\[
\begin{array}{c|cc}
...
1{,}50 & 0{,}0162 & 85{,}376\\
1{,}80 & 0{,}0058 & 109{,}121\\
2{,}10 & \approx 0 & 28{,}841
\end{array}
\]
Les références indépendantes sont cohérentes~: le splitting AMS donne $0{,}1387$ et le MC
direct donne $0{,}1390$ ($27\,802/200\,000$ succès). En revanche, les grands tilts
produisent des poids instables qui font diverger le CV~; ils ne doivent pas être
interprétés comme de meilleurs estimateurs. La chute du CV observée à $\theta=2{,}10$
est trompeuse~: elle provient du fait que presque aucune trajectoire n'atteint
$\tau_b<\tau_a$ sous ce tilt trop agressif, faisant tendre $\hat p$ vers zéro.
...
\begin{resultatpositif}[title={IS optimal ($\theta^*\approx0{,}30$) comparable à l'AMS sur ce problème}]
Un tilt modéré ($\theta^*\approx0{,}30$) réduit le CV de $2{,}52$ à $2{,}05$
($-19\,\%$ par trajectoire). La comparaison directe CLT à $N$ fixé (§~\ref{sec:clt_comparison})
montre que, au même $N$, l'IS avec $\theta^*$ atteint même une efficacité supérieure
à l'AMS ($\mathrm{RE}=0{,}57 < 1$, voir ci-après). Toutefois, les grands tilts
($\theta>0{,}60$) dégénèrent rapidement~: l'IS reste donc utile comme contrôle de
vraisemblance et comme estimateur alternatif au voisinage de $\theta^*$.
\end{resultatpositif}
\end{demonstration}

\subsection{Comparaison directe CLT~: AMS vs IS}
\label{sec:clt_comparison}

Pour quantifier la question «~quel estimateur est le plus précis à $N$ fixé~?~»,
on effectue $R=50$ réplications indépendantes de chaque méthode avec $N=500$
trajectoires/particules, puis on mesure l'écart-type empirique des estimées
$\hat P$ et la largeur des IC 95\,\% en fonction de $N$.

\begin{figure}[H]
\centering
\reportfig[width=0.92\linewidth]{clt_comparison.png}
\caption{Comparaison CLT~: AMS (orange) vs IS $\theta=0{,}30$ (violet).
\textbf{Gauche}~: histogrammes de $\hat P$ sur $R=50$ réplications à $N=500$~;
le pic plus étroit de l'IS traduit une variance plus faible.
\textbf{Droite}~: largeur d'IC 95\,\% en fonction de $N$~; les deux courbes
décroissent en $1/\sqrt{N}$ (ligne pointillée de référence).
\textit{Horiz.~: $\hat{P}$~; $N$~; vert.~: nombre de réplications~; largeur IC.}}
\label{fig:clt_comparison}
\end{figure}

\paragraph{Résultats quantitatifs.}
Sur les $R=50$ réplications à $N=500$~:
...
\midrule
Splitting AMS & $0{,}01712$ & $1{,}00$ (référence) \\
IS ($\theta^*=0{,}30$) & $0{,}01289$ & $0{,}57$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent où $\mathrm{RE}=(\sigma_\mathrm{IS}/\sigma_\mathrm{AMS})^2=0{,}57 < 1$~: à nombre
de trajectoires/particules identique, l'IS avec $\theta^*=0{,}30$ requiert environ
$43\,\%$ moins de réplications que l'AMS pour atteindre la même précision sur ce problème.

Ce résultat complète la conclusion du §~\ref{sec:is}~: le tilt exponentiel modéré
ne peut pas exploiter la géométrie par niveaux de l'événement rare (contrairement
au splitting), mais il tilte suffisamment les trajectoires vers la zone rare pour
surpasser l'AMS à $N$ modéré. L'AMS reprend l'avantage pour les événements très
profonds ($k\ge5$) où sa structure par niveaux est indispensable.
...
Black Thursday correspond à une cascade bid rare. 
\end{resultatpositif}
...
sampling par basculement exponentiel fournit un contrôle indépendant~: à $N$
fixé, l'IS avec $\theta^*=0{,}30$ est même plus efficace que l'AMS
($\mathrm{RE}=0{,}57$), mais sans exploiter la structure par niveaux, il perd
cet avantage pour les flash crashes profonds.
